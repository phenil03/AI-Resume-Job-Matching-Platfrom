...
[Your Name]`;
  ...
  const getStatusBadge = (status: Message['status']) => {
    const styles = {
      'Interview Request': 'bg-[#F8F9FB] text-[#1D9E75] border-[#1D9E75]/30',
      'Interested': 'bg-[#1D9E75]/20 text-[#085041] border-[#1D9E75]/30',
      'Rejected': 'bg-[#FCEBEB] text-[#791F1F] border-[#791F1F]/20',
      'Follow Up': 'bg-yellow-500/20 text-[#633806] border-yellow-500/30'
    };
    return (
    ...
    <div className="space-y-4">
      {messages.map((message) => {
        ...
        <motion.div
          ...
          className={`relative bg-[#FFFFFF] border border-[#E8E8E8] rounded-[12px] overflow-hidden transition-all duration-300 ${
            !message.isRead ? 'shadow-[L-4px_0_0_0_#3b82f6]' : ''
          } ${expandedId === message.id ? 'bg-[#FFFFFF] ring-1 ring-[#E8E8E8]' : 'hover:bg-[#F8F9FB]'}`}
        >
          {/* Unread Glow Border */}
          {!message.isRead && (
            <div className="absolute top-0 left-0 bottom-0 w-1 bg-[#1D9E75] shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
          )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-[12px] flex items-center justify-center shrink-0 ${
                  !message.isRead ? 'bg-[#F8F9FB] text-[#1D9E75]' : 'bg-[#FFFFFF] text-[#AAAAAA]'
                }`}>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-[#444444] mb-1">{message.jobTitle}</p>
                ...
                <AnimatePresence mode="wait">
                  {expandedId !== message.id && (
                    <motion.p 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="text-sm text-[#888888] italic max-w-md truncate"
                    >
                      "{message.preview}"
                    </motion.p>
                  )}
              ...
              {expandedId === message.id && (
                ...
                >
                  <div className="mt-6 pt-6 border-t border-[#E8E8E8]">
                    <p className="text-[#444444] leading-relaxed mb-6 bg-[#FFFFFF] p-4 rounded-[12px] border border-[#E0E0E0] italic">
                      "{message.fullMessage}"
                    </p>
                    <div className="mb-5">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <h4 className="text-sm font-bold text-[#444444]">Suggested Reply</h4>
                        <span className="text-[10px] uppercase tracking-widest text-[#AAAAAA]">Editable Template</span>
                      </div>
                      <textarea
                        value={replyText}
                        onClick={(e) => e.stopPropagation()}
                        onChange={(e) => setDrafts(prev => ({ ...prev, [message.id]: e.target.value }))}
                        className="w-full min-h-[190px] resize-none bg-[#FFFFFF] border border-[#E8E8E8] rounded-[12px] p-4 text-sm leading-relaxed text-[#444444] outline-none"
                      />
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      <button
                        ...
                        className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 rounded-[12px] text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/20 inline-flex items-center gap-2"
                      >
                      ...
                      <a 
                        ...
                        onClick={(e) => e.stopPropagation()}
                        className="px-5 py-2.5 bg-[#1D9E75] hover:bg-[#1D9E75] rounded-[12px] text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-[#1D9E75]/20 inline-flex items-center gap-2"
                      >
                      ...
                      <a 
                        href={emailHref}
                        onClick={(e) => e.stopPropagation()}
                        className="px-5 py-2.5 bg-[#FFFFFF] hover:bg-[#FFFFFF] border border-[#E8E8E8] rounded-[12px] text-xs font-bold transition-all inline-flex items-center gap-2"
                      >
                      ...
                      <button 
                        ...
                        className="px-6 py-2.5 bg-[#FFFFFF] hover:bg-[#FFFFFF] border border-[#E8E8E8] rounded-[12px] text-xs font-bold transition-all"
                      >
                        Close
                      </button>
                    </div>
